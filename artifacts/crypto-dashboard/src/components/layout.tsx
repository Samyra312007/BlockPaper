...
import { Activity, LogOut, Briefcase, History, LineChart, Brain, Users, Trophy, FlaskConical } from "lucide-react";
      ...
      <header className="h-14 border-b border-border flex items-center justify-between px-4 shrink-0 bg-card">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex items-center gap-1">
            ...
            <Link href="/quests" className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${location === '/quests' ? 'bg-secondary text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'}`}>
              <span className="flex items-center gap-2"><Trophy className="h-4 w-4"/> Quests</span>
            </Link>
            <Link href="/backtest" className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${location === '/backtest' ? 'bg-secondary text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'}`}>
              <span className="flex items-center gap-2"><FlaskConical className="h-4 w-4"/> Backtest</span>
            </Link>
          </nav>
        </div>
